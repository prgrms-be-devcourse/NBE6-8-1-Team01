...
export const wishlistApi = {
  ...
  getWishlist: (email: string) => 
    apiCall<ApiResponse<WishList[]>>(`/api/v1/wishlists/${email}`),
  ...
  updateWishlist: (email: string, wishId: number, updateData: WishListUpdate) => 
    apiCall<ApiResponse<WishList>>(`/api/v1/wishlists/${email}/${wishId}`, {
      method: 'PUT',
  ...
  removeFromWishlist: (email: string, wishId: number) => 
    apiCall<ApiResponse<void>>(`/api/v1/wishlists/${email}/${wishId}`, {
      method: 'DELETE',
    }),
}
